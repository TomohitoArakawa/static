...
var gnaviCurrent = function() {
	var dirName = location.pathname.split('/')[1],
		...
		$('[data-target="grpHeaderGnavCurrent"]').find('a[href="'+ href +'"]').parent('li').addClass('current');
};
...
var fixedHeader = function() {
	...
	if(!unFixed) {
		var $window = $(window),
			$el = $('[data-target="grpHeaderFixedBar"]'),
			$parts = $el.find($('[data-target="grpHeaderFixedParts"]')),
			startHeight = 400,
			speed = 200,
			$fixed, isFixed;
